...
let CoreTestList =
    TestList("Core", [
        Test.Sync("Guid.FromInt is predefined, a new guid is not", fun () ->
            ...
            Assert.True(not (Guid.NewGuid().IsPredefined), "randomly generated"))
        Test.Sync("Guid.FromInt is injective on the int", fun () ->
            Assert.Equal(Guid.FromInt 7, Guid.FromInt 7)
            Assert.NotEqual(Guid.FromInt 7, Guid.FromInt 8, "different ints, different guids"))
        ...
        Test.Sync("withLock releases the lock when the body throws", fun () ->
            ...
            Assert.True(not l.IsHeldByCurrentThread, "the scope was disposed"))
        Test.Sync("withLock holds the lock for the duration of the body", fun () ->
            ...
            Assert.True(not l.IsHeldByCurrentThread, "released after"))
        Test.Sync("NonGenericWorkaround compares matching types", fun () ->
            ...
            Assert.True(not (NonGenericWorkaround.equals<string>("a", box "b")), "different strings")
